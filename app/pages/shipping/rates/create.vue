...
const initialValues = { shipping_zone_id: "", shipping_method_id: "", rate_type: "", amount: 0 };
...
async function onSubmit({ valid, values }) {
  ...
  try {
    ...
    if (freeOverAmount.value !== null) payload.free_over_amount = freeOverAmount.value;
    if (minOrderAmount.value !== null) payload.min_order_amount = minOrderAmount.value;
    if (maxOrderAmount.value !== null) payload.max_order_amount = maxOrderAmount.value;
    if (estimatedDaysMin.value !== null) payload.estimated_days_min = estimatedDaysMin.value;
    if (estimatedDaysMax.value !== null) payload.estimated_days_max = estimatedDaysMax.value;

    const response = await rateStore.createRate(payload);
    navigateTo(`/shipping/rates/${response.data.id}/edit`);
  } catch (e) {
...
<template>
  <div>
    <div class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-2xl font-bold text-slate-900">Create Shipping Rate</h1>
    </div>
    ...
    <Form v-else v-slot="$form" :initial-values="initialValues" :resolver="resolver" @submit="onSubmit">
      ...
      <!-- Optional thresholds + delivery estimate -->
      <div class="bg-white rounded-lg border border-slate-200 p-6 mb-6">
      ...
      <div class="flex justify-end">
        <Button type="submit" label="Create Rate" icon="pi pi-check" :loading="loading" />
      </div>
    </Form>
  </div>
</template>

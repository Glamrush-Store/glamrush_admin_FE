...
async function onSubmit({ valid, values }) {
  ...
  try {
    ...
    const response = await store.createPaymentMethod(payload);
    navigateTo(`/settings/payment-methods/${response.data.id}/edit`);
  } catch (e) {
...
<template>
  <div>
    <div class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-2xl font-bold text-slate-900">Create Payment Method</h1>
    </div>
    ...
    <Form v-slot="$form" :initial-values="initialValues" :resolver="resolver" @submit="onSubmit">
      ...
      <div class="flex justify-end">
        <Button type="submit" label="Create Payment Method" icon="pi pi-check" :loading="loading" />
      </div>
    </Form>
  </div>
</template>
